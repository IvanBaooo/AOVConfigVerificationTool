...
-> 保存区域缓存
-> 公共规则与区域覆盖规则合并
-> 打包时后端白名单/高危名单与本地设置页名单并集合并（下发在前、本地追加在后）
-> Report 和网页归档记录规则版本
```
...
`whitelist_paths`（提交告警白名单）与 `high_risk_paths`（提交高危名单）在通用层
`common` 和分区 `regions` 都可配置，分区条目追加在通用条目之后合并进有效规则。
打包时下发名单与本地设置页名单（`commit_whitelist` / `commit_high_risk`）并集合并：
下发条目在前、本地条目追加在后，按归一化路径去重；下发列表为空时本地配置保持不动。
高危名单命中的提交会产生需人工确认的 `commit_high_risk_confirm` check。
...
后续只需发布新版本补充路径映射、白名单和高危名单，无需修改本地打包程序。
...
- 维护路径映射、提交告警白名单和提交高危名单，不直接编辑原始 JSON。
- 实时校验必填项、路径格式和重复规则。
- 发布前展示新增、修改、删除和问题数量，并要求人工二次确认。

### 表内容校验规则

规则编辑器中的“表内容校验”与 SVN 路径映射、白名单、高危名单使用同一版本。
